...
class TrotGaitController(GaitController):
    def __init__(self, default_stance, stance_time, swing_time, time_step, use_imu):
        ...
        self.max_x_velocity = 0.02 #[m/s]
        self.max_y_velocity = 0.003 #[m/s] 
    ...
    def step(self, state, command):
        ...
        if self.autoRest:
            if abs(command.velocity[0]) <= 0.005 and abs(command.velocity[1]) <= 0.005 and abs(command.yaw_rate) <= 0.002:
        ...
        if self.trotNeeded:
            ...
            new_foot_locations = np.zeros((3,8))
...
class TrotSwingController(object):
    ...
    def swing_height(self, swing_phase):
        ...
        swing_height_ = self.z_leg_lift*math.sqrt(1 - (swing_phase - 0.5)**2/0.25)
