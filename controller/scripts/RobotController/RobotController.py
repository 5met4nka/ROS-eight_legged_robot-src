...
class Robot(object):
    def __init__(self, body, legs, imu):
        ...
        self.delta_x1 = self.body[0] * 0.5 + (self.legs[0] + self.legs[1] + self.legs[2])*sin(pi/4)
        self.delta_y1 = self.body[1] * 0.5 + (self.legs[0] + self.legs[1] + self.legs[2])*cos(pi/4)
        self.delta_x2 = 0.015 +              (self.legs[0] + self.legs[1] + self.legs[2])*sin(pi/12)
        self.delta_y2 = self.body[2] * 0.5 + (self.legs[0] + self.legs[1] + self.legs[2])*cos(pi/12)
        self.default_height = self.legs[3] - 0.01625
        #self.x_shift_front = -0.1
        #self.x_shift_back = -0.02   

        print(f"self.delta_x1 : {self.delta_x1}, self.delta_y1 : {self.delta_y1}, self.delta_y2 : {self.delta_y2}, self.default_height : {self.default_height}")
    ...
    def default_stance(self):
        # FR, FL, MR1, ML1, MR2, ML2, RR, RL
        return np.array([[self.delta_x1, self.delta_x1 , self.delta_x2, self.delta_x2 ,-self.delta_x2, -self.delta_x2,-self.delta_x1, -self.delta_x1],
                         [self.delta_y1, -self.delta_y1, self.delta_y2, -self.delta_y2, self.delta_y2, -self.delta_y2, self.delta_y1, -self.delta_y1],
                         [0            , 0             , 0            , 0             , 0            , 0             , 0            , 0            ]])
