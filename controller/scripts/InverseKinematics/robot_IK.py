...
class InverseKinematics(object):
    ...
    def get_local_positions(self,leg_positions,dx,dy,dz,roll,pitch,yaw):
        ...
        leg_positions = (np.block([[leg_positions],[np.array([1,1,1,1,1,1,1,1])]])).T
        ...
        # Transformation matrix, base_link_world => MR1
        T_blwMR1 = np.dot(T_blwbl, homog_transform(0.015,
                          +0.5*self.bodyWidth2,0,0,0,0))

        # Transformation matrix, base_link_world => ML1
        T_blwML1 = np.dot(T_blwbl, homog_transform(0.015,
                          -0.5*self.bodyWidth2,0,0,0,0))

        T_blwMR2 = np.dot(T_blwbl, homog_transform(-0.015,
                          +0.5*self.bodyWidth2,0,0,0,0))

        # Transformation matrix, base_link_world => ML1
        T_blwML2 = np.dot(T_blwbl, homog_transform(-0.015,
                          -0.5*self.bodyWidth2,0,0,0,0))
        ...
        pos_MR1 = np.dot(homog_transform_inverse(T_blwMR1),leg_positions[2])
        pos_ML1 = np.dot(homog_transform_inverse(T_blwML1),leg_positions[3])       
        pos_MR2 = np.dot(homog_transform_inverse(T_blwMR2),leg_positions[4])
        pos_ML2 = np.dot(homog_transform_inverse(T_blwML2),leg_positions[5])   
        pos_RR = np.dot(homog_transform_inverse(T_blwRR1),leg_positions[6])
        pos_RL = np.dot(homog_transform_inverse(T_blwRL1),leg_positions[7])
    ...
    def inverse_kinematics(self,leg_positions,dx,dy,dz,roll,pitch,yaw):
        ...
        for i in range(8):
            ...
            if(i == 0 or i == 1):
                ...
                theta1 = atan2(x,y) - pi*((2.5-i)*2)/12 - (i-2)*pi
            elif(i == 4 or i == 5):  
                theta1 = atan2(x,y) + pi*((4.5-i)*2)/12 + (i-4)*pi  
            else:
                theta1 = atan2(x,y) + pi/4*(1+2*(i-6))
